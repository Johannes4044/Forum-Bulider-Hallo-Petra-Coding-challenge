...
import { updateForm, getForm } from './actions'
...
export default function EditFormPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        async function loadForm() {
            ...
            const result = await getForm(resolvedParams.id)
            if (result.success && result.form) {
                setTitle(result.form.title)
                setDescription(result.form.description || '')
                setFields(result.form.fields.map((field: any) => ({
                    tempId: field.id,
                    id: field.id,
                    key: field.key,
                    label: field.label,
    ...
    if (isLoading) {
        return (
            <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom right, #f0f9ff, #e0f2fe, #f0f9ff)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <main style={{ padding: '48px 20px', maxWidth: '1200px', margin: '0 auto' }}>
                <div style={{
                    ...
                    <p style={{
                        ...
                        Update and refine your form fields
                    </p>
                </div>
